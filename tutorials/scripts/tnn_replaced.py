...
def mnn_tnn(ds1, ds2, names1, names2, knn = 20,lr=1e-3,default_root_dir='./Logs/tnn_supervised/',min_ppf=0.85,min_percentile=95, min_value=0.8,percent=50,lamb = 0.3,transformed=False,transformed_datas=None,ckpt_dir=None,optimizer = 'adam',only_mnn=False,match=None,devices=[0],scale=False):
    '''
...
def generator_from_index(adata, batch_name,  celltype_name=None, mask_batch=None, Y = None, k = 20, label_ratio = 0.8, k_to_m_ratio = 0.75, batch_size = 32, search_k=-1,
                         save_on_disk = True, approx = True, verbose=1,ckpt_dirs=None):
    ...
    mnn_dict = create_dictionary_mnn(adata, batch_name=batch_name, k = k, save_on_disk = save_on_disk, approx = approx, verbose = verbose,ckpt_dirs=ckpt_dirs)
...
def create_dictionary_mnn(adata, batch_name, k = 50, save_on_disk = True, approx = True, verbose = 1,ckpt_dirs=None):
    ...
    for comb in list(itertools.combinations(range(len(cells)), 2)):
        ...
        match_mnn = mnn(normalize(ds1), normalize(ds2), names1, names2, knn=k, save_on_disk = save_on_disk, approx = approx)
            
        if ckpt_dirs is None:
            match = mnn_tnn(ds1, ds2, names1, names2, knn=20,match = match_mnn)
        else:
            match = mnn_tnn(ds1, ds2, names1, names2, knn=20,ckpt_dir=ckpt_dirs[ckpt_count],match = match_mnn)
        ckpt_count +=1
...
def create_dictionary_knn(adata, cell_subset, k = 50, save_on_disk = True, approx = True):
    ...
    if approx:
        ...
        p.init_index(max_elements=num_elements, ef_construction=100,M=16 )#M=16
        p.set_ef(10)
        p.add_items(pcs)
...
class TNN(BaseEstimator):
    ...
    def _fit(self, X, batch_name, celltype_name=None, mask_batch=None, Y=None, shuffle_mode=True,ckpt_dirs=None):
        ...
        datagen = generator_from_index(X,
                                       ...
                                       approx = self.approx,ckpt_dirs=ckpt_dirs
                                       )
    ...
    def fit(self, X, batch_name, celltype_name=None, mask_batch=None, Y=None, shuffle_mode=True,ckpt_dirs = None):
        """Fit model.
        Parameters
        ----------
        ...
        """
        self._fit(X, batch_name, celltype_name, mask_batch, Y, shuffle_mode,ckpt_dirs = ckpt_dirs)
        return self
