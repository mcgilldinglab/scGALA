...
import hnswlib
from scGALA import mnn_tnn,mnn_tnn_spatial
...
def create_dictionary_mnn_mod(adata, use_rep, batch_name, k = 50, save_on_disk = True, approx = True, verbose = 1, iter_comb = None,ckpt_dir=None,device=[0],spatial=False):
    ...
    for comb in iter_comb:
        ...
        # match = mnn(ds1, ds2, names1, names2, knn=k, save_on_disk = save_on_disk, approx = approx)
        if spatial:
            spatial1 = adata[new].obsm['spatial']
            spatial2 = adata[ref].obsm['spatial']
            match = mnn_tnn_spatial(ds1, ds2,spatial1,spatial2, names1, names2, knn=k,ckpt_dir=ckpt_dir,only_mnn=False,match=None,devices=device,scale=True,min_value=0.9,lamb = 0.3)
        else:
            match = mnn_tnn(ds1, ds2, names1, names2, knn=k,ckpt_dir=ckpt_dir,only_mnn=False,match=None,devices=device,scale=True,min_value=0.9,lamb = 0.3)
