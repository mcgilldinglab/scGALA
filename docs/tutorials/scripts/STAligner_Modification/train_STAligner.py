...
from .mnn_utils import create_dictionary_mnn,create_dictionary_mnn_mod
...
def train_STAligner_subgraph_mod(adata, hidden_dims=[512, 30], n_epochs=1000, lr=0.001, key_added='STAligner',
                             gradient_clipping=5., weight_decay=0.0001, margin=1.0, verbose=False,
                             random_seed=666, iter_comb=None, knn_neigh=100, Batch_list=None,
                             device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'),ckpt_dirs=None,spatial=False):
    """\
    Train graph attention auto-encoder and use spot triplets across slices to perform batch correction in the embedding space.
    To deal with large-scale data with multiple slices and reduce GPU memory usage, each slice is considered as a subgraph for training.
        ...
        if epoch % 100 == 0 or epoch == 500:
            ...
            for comb in iter_comb:
                ...
                if ckpt_dirs is not None:
                    mnn_dict = create_dictionary_mnn_mod(batch_pair, use_rep='STAGATE', batch_name='batch_name',
                                                           k=knn_neigh,
                                                           iter_comb=None, verbose=0,ckpt_dir=ckpt_dirs[ckpt_index],device=[1],spatial=spatial)
                else:
                    mnn_dict = create_dictionary_mnn_mod(batch_pair, use_rep='STAGATE', batch_name='batch_name',
                                                           k=knn_neigh,
                                                           iter_comb=None, verbose=0,device=[1],spatial=spatial)
                ckpt_index += 1
